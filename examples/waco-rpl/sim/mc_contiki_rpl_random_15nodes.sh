...
# Monte Carlo for contiki-rpl random 15 nodes
# Usage: mc_contiki_rpl_random_15nodes.sh [NUM_SEEDS] [OUTDIR]
# - NUM_SEEDS: số lần chạy (mặc định 10)
# - OUTDIR: thư mục lưu kết quả (mặc định examples/waco-rpl/sim/out/contiki-rpl-random-15-nodes-mc)
...
OUTDIR=${2:-"$ROOT_DIR/examples/waco-rpl/sim/out/contiki-rpl-random-15-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-random-15-nodes/contiki-rpl-random-15-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-random-15-nodes"
BASENAME="contiki-rpl-random-15-nodes"
