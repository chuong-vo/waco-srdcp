...
# Monte Carlo headless runs cho kịch bản RANDOM 5-node (waco-rpl)
# Usage: mc_rpl_random_5nodes.sh [NUM_SEEDS] [OUTDIR]
# - NUM_SEEDS: số lần chạy (mặc định 10)
# - OUTDIR: thư mục lưu kết quả (mặc định examples/waco-rpl/sim/out/waco-rpl-random-5-nodes-mc)
...
OUTDIR=${2:-"$ROOT_DIR/examples/waco-rpl/sim/out/waco-rpl-random-5-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-rpl/sim/waco-rpl-random-5-nodes/waco-rpl-random-5-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-rpl/sim/waco-rpl-random-5-nodes"
BASENAME="waco-rpl-random-5-nodes"
