...
# Monte Carlo for contiki-rpl chain 5 nodes
# Usage: mc_contiki_rpl_chain_5nodes.sh [NUM_SEEDS] [OUTDIR]

NUM_SEEDS=${1:-10}
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../../.." && pwd)"
OUTDIR=${2:-"$ROOT_DIR/examples/waco-rpl/sim/out/contiki-rpl-chain-5-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-chain-5-nodes/contiki-rpl-chain-5-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-chain-5-nodes"
BASENAME="contiki-rpl-chain-5-nodes"

echo "[mcR5] OUTDIR: $OUTDIR"; mkdir -p "$OUTDIR"
ant -q -f "$COOJA_BUILD_XML" jar >/dev/null

for s in $(seq 1 "$NUM_SEEDS"); do
  echo "[mcR5] Seed $s / $NUM_SEEDS: run"
  ...
  [[ -f "$latest_txt" ]] || { echo "[mcR5][ERR] no TXT after seed=$s" >&2; exit 1; }
