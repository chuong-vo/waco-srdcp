...
# Monte Carlo for contiki-rpl random 5 nodes
# Usage: mc_contiki_rpl_random_5nodes.sh [NUM_SEEDS] [OUTDIR]
# - NUM_SEEDS: số lần chạy (mặc định 10)
# - OUTDIR: thư mục lưu kết quả (mặc định examples/waco-rpl/sim/out/contiki-rpl-random-5-nodes-mc)
...
OUTDIR=${2:-"$ROOT_DIR/examples/waco-rpl/sim/out/contiki-rpl-random-5-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-random-5-nodes/contiki-rpl-random-5-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-rpl/sim/contiki-rpl-random-5-nodes"
BASENAME="contiki-rpl-random-5-nodes"

printf '[mcRR5] Chuẩn bị thư mục output: %s
' "$OUTDIR"
mkdir -p "$OUTDIR"
...
' "$s"
  ...
  if [[ -f "$OUTDIR/seed-${s}_dc.txt" ]]; then
    printf '[mcRR5] Seed %s: tính năng lượng (radio) -> CSV
' "$s"
...
printf '[mcRR5] Hoàn tất. Kết quả nằm ở: %s
' "$OUTDIR"
printf '[mcRR5] Gợi ý: xem network_avgs.csv, energy_network_avgs.csv, per_node_avg.csv, per_node_energy_avg.csv
'
