...
# Monte Carlo headless runs for the 5-node chain scenario
# Usage: mc_5nodes.sh [NUM_SEEDS] [OUTDIR]
# - NUM_SEEDS: số lần chạy (mặc định 10)
# - OUTDIR: thư mục lưu kết quả (mặc định examples/waco-srdcp/sim/out/waco-srdcp-chain-5-nodes-mc)
...
OUTDIR=${2:-"$ROOT_DIR/examples/waco-srdcp/sim/out/waco-srdcp-chain-5-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-srdcp/sim/waco-srdcp-chain-5-nodes/waco-srdcp-chain-5-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-srdcp/sim/waco-srdcp-chain-5-nodes"
...
for s in $(seq 1 "$NUM_SEEDS"); do
  ...
  latest_txt=$(ls -t "$LOGDIR"/waco-srdcp-chain-5-nodes-*.txt 2>/dev/null | grep -v '_dc\.txt' | head -1 || true)
  latest_dc=$(ls -t "$LOGDIR"/waco-srdcp-chain-5-nodes-*_dc.txt 2>/dev/null | head -1 || true)
  if [[ -z "${latest_txt}" ]]; then
    # fallback không có suffix
    latest_txt="$LOGDIR/waco-srdcp-chain-5-nodes.txt"
    latest_dc="$LOGDIR/waco-srdcp-chain-5-nodes_dc.txt"
  fi
