...
# Monte Carlo headless runs for the 30-node grid scenario
# Usage: mc_grid_30nodes.sh [NUM_SEEDS] [OUTDIR]
# - NUM_SEEDS: số lần chạy (mặc định 10)
# - OUTDIR: thư mục lưu kết quả (mặc định examples/waco-srdcp/sim/out/waco-srdcp-grid-30-nodes-mc)
...
OUTDIR=${2:-"$ROOT_DIR/examples/waco-srdcp/sim/out/waco-srdcp-grid-30-nodes-mc"}

COOJA_BUILD_XML="$ROOT_DIR/tools/cooja/build.xml"
SCEN="$ROOT_DIR/examples/waco-srdcp/sim/waco-srdcp-grid-30-nodes/waco-srdcp-grid-30-nodes.csc"
LOGDIR="$ROOT_DIR/examples/waco-srdcp/sim/waco-srdcp-grid-30-nodes"

echo "[mcG30] Chuẩn bị thư mục output: $OUTDIR"
mkdir -p "$OUTDIR"

if ! command -v ant >/dev/null 2>&1; then
  echo "[mcG30][ERR] Không tìm thấy 'ant' trong PATH" >&2
  exit 1
fi

if ! command -v python3 >/dev/null 2>&1; then
  echo "[mcG30][ERR] Không tìm thấy 'python3' trong PATH" >&2
  exit 1
fi

echo "[mcG30] Build cooja jar (nếu cần)"
ant -q -f "$COOJA_BUILD_XML" jar >/dev/null

for s in $(seq 1 "$NUM_SEEDS"); do
  echo "[mcG30] Seed $s / $NUM_SEEDS: chạy headless..."
  ...
  latest_txt=$(ls -t "$LOGDIR"/waco-srdcp-grid-30-nodes-*.txt 2>/dev/null | grep -v '_dc\.txt' | head -1 || true)
  latest_dc=$(ls -t "$LOGDIR"/waco-srdcp-grid-30-nodes-*_dc.txt 2>/dev/null | head -1 || true)
  if [[ -z "${latest_txt}" ]]; then
    # fallback không có suffix
    latest_txt="$LOGDIR/waco-srdcp-grid-30-nodes.txt"
    latest_dc="$LOGDIR/waco-srdcp-grid-30-nodes_dc.txt"
  fi

  if [[ ! -f "$latest_txt" ]]; then
    echo "[mcG30][ERR] Không tìm thấy log TXT sau khi chạy seed=$s" >&2
    exit 1
  fi

  dest_txt="$OUTDIR/seed-$s.txt"
  echo "[mcG30] Seed $s: lưu log -> $dest_txt"
  echo "[mcG30]   nguồn TXT: $latest_txt"
  mv "$latest_txt" "$dest_txt"
  if [[ -n "${latest_dc}" && -f "$latest_dc" ]]; then
    echo "[mcG30]   nguồn DC:  $latest_dc"
    mv "$latest_dc" "$OUTDIR/seed-${s}_dc.txt"
  fi

  echo "[mcG30] Seed $s: parse chỉ số -> CSV"
  ...
  if [[ -f "$OUTDIR/seed-${s}_dc.txt" ]]; then
    echo "[mcG30] Seed $s: tính năng lượng (radio) -> CSV"
...
echo "[mcG30] Hoàn tất. Kết quả nằm ở: $OUTDIR"
echo "[mcG30] Gợi ý: xem $agg, $eagg hoặc các file *_summary.csv/_network_avg.csv theo seed."
