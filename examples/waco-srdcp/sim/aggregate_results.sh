...
if [[ -e "${sum_files[0]:-}" ]]; then
  python3 "$ROOT_DIR/examples/waco-srdcp/sim/agg_per_node.py" --out "$OUTDIR/per_node_avg.csv" "${sum_files[@]}" || true
...
if [[ -e "${enode_files[0]:-}" ]]; then
  python3 "$ROOT_DIR/examples/waco-srdcp/sim/agg_per_node.py" --out "$OUTDIR/per_node_energy_avg.csv" "${enode_files[@]}" || true
