...
/**
 * @brief Extracts a two-byte ID from a link-layer address for consistent logging.
 * @param a   A pointer to the link-layer address.
 * @param[out] id0 A pointer to store the first byte of the ID.
 * @param[out] id1 A pointer to store the second byte of the ID.
 * @details For TSCH (8-byte EUI-64), this uses bytes 6 and 7.
 */
...
/**
 * @brief Prints the two-byte ID representation of an address into a buffer.
 * @param a   A pointer to the link-layer address.
 * @param[out] buf The output character buffer.
...
/**
 * @brief Prints the two-byte ID of a parent's address into a buffer.
 * @param p   A pointer to the parent's link-layer address.
 * @param[out] buf The output character buffer.
 * @param n   The size of the buffer.
 */
...
/**
 * @brief Looks up or creates an entry in the ID-to-IPv6 address map.
 * @param id0    The first byte of the node's ID.
 * @param id1    The second byte of the node's ID.
 * @param create If non-zero, create a new entry if one is not found.
 * @return A pointer to the map entry, or NULL if not found and not created.
 * @details This map is used by the sink to learn the IPv6 addresses of nodes for downlink traffic.
 */
static id_ip_t *
id_ip_lookup(uint8_t id0, uint8_t id1, uint8_t create)
{
  int i;
  int free_slot = -1;
  for (i = 0; i < MAP_MAX_NODES; i++)
  {
    if (id_ip_map[i].known)
    {
      if (id_ip_map[i].id0 == id0 && id_ip_map[i].id1 == id1)
      {
        return &id_ip_map[i];
      }
    }
  ...
  {
    id_ip_map[free_slot].known = 1;
    id_ip_map[free_slot].id0 = id0;
    id_ip_map[free_slot].id1 = id1;
    return &id_ip_map[free_slot];
  }
  return NULL;
}

/**
 * @brief Finds an entry in the ID-to-IPv6 map by the last byte of its ID.
 * @param id1 The last byte of the node's ID (which corresponds to the `node_id`).
 * @return A pointer to the map entry, or NULL if not found.
 * @details This is a convenience function for the sink to find a destination for downlink traffic.
 */
static id_ip_t *
id_ip_find_by_last(uint8_t id1)
{
  int i;
  for (i = 0; i < MAP_MAX_NODES; i++)
  {
    if (id_ip_map[i].known && id_ip_map[i].id1 == id1)
    {
      return &id_ip_map[i];
    }
  }
...
/**
 * @brief Finds or adds an entry for a source in the uplink PDR statistics table.
 * @param id0 The first byte of the source's ID.
 * @param id1 The second byte of the source's ID.
 * @return A pointer to the PDR entry, or NULL if the table is full.
 * @details If the source is not found, a new entry is created.
 */
...
/**
 * @brief Resets PDR statistics when a sequence number wrap-around is detected.
 * @param st  A pointer to the PDR statistics entry.
 * @param seq The current sequence number.
 * @details This prevents incorrect PDR calculation when a node restarts its sequence.
 */
...
/**
 * @brief Updates the uplink PDR statistics for a given source.
 * @param id0 The first byte of the source's ID.
 * @param id1 The second byte of the source's ID.
 * @param seq The sequence number of the received packet.
 * @details This function tracks received packets, gaps, and duplicates.
 */
...
/**
 * @brief Prints the CSV header for general INFO lines, only once at boot.
 */
...
/**
 * @brief Prints a CSV INFO line with the node's current role and status.
 * @param role   A string describing the role ("SINK" or "NODE").
 * @param metric The node's current RPL metric (approximated hops).
 * @param parent A pointer to the parent's link-layer address.
 */
...
/**
 * @brief Prints the uplink PDR statistics in CSV format at the SINK.
 * @param my_metric The SINK's metric (always 0).
 * @param parent    The SINK's parent (always NULL).
 */
static void pdr_ul_print_csv(uint16_t my_metric, const linkaddr_t *parent)
{
  ...
  for (i = 0; i < PDR_MAX_SRC; i++)
  {
    if (pdr_ul[i].used)
    {
      ...
      /* Alias PRR_UL = PDR_UL (with newline) so parsers can read a value */
...
/**
 * @brief Resets downlink PDR statistics when a sequence number wrap-around is detected.
 * @param seq The current sequence number.
 * @details This prevents incorrect PDR calculation when the sink restarts its sequence.
 */
...
/**
 * @brief Updates the self-assessed downlink PDR statistics at the NODE.
 * @param seq The sequence number of the received packet.
 * @details This function tracks received packets, gaps, and duplicates from the sink.
 */
...
/**
 * @brief Prints the downlink PDR statistics in CSV format at the NODE.
 * @param my_metric The node's current RPL metric (approximated hops).
 * @param parent    A pointer to the node's parent's link-layer address.
 * @param sink      A pointer to the sink's link-layer address.
 */
static void pdr_dl_print_csv(uint16_t my_metric, const linkaddr_t *parent, const linkaddr_t *sink)
{
  ...
  /* Alias PRR_DL = PDR_DL (with newline) */
...
/**
 * @brief Approximates the number of hops to the root based on RPL rank.
 * @return The approximate hop count, or 0xFFFF if not in a DAG.
 * @details This divides the node's rank by the minimum hop rank increase.
 */
...
/**
 * @brief Callback for receiving an uplink (UL) UDP packet.
 * @param c             The UDP connection.
 * @param sender_addr   The IPv6 address of the sender.
 * @param sender_port   The sender's port.
 * @param receiver_addr The receiver's IPv6 address.
 * @param receiver_port The receiver's port.
                     ...
                     uint16_t receiver_port, const uint8_t *data, uint16_t datalen)
{
  if (datalen >= sizeof(ul_msg_t))
  {
    ...
    if (node_id == 1)
    {
      id_ip_t *entry = id_ip_lookup(m->src0, m->src1, 1);
      if (entry)
      {
        uip_ipaddr_copy(&entry->ip6, sender_addr);
      }
      /* Update PDR-UL stats */
...
/**
 * @brief Callback for receiving a downlink (DL) UDP packet.
 * @param c             The UDP connection.
 * @param sender_addr   The IPv6 address of the sender (the sink).
 * @param sender_port   The sender's port.
 * @param receiver_addr The receiver's IPv6 address (this node).
 * @param receiver_port The receiver's port.
 * @param data          A pointer to the received data.
 * @param datalen       The length of the received data.
 */
...
PROCESS(waco_rpl_process, "WaCo + TSCH RPL UDP example");
...
PROCESS_THREAD(waco_rpl_process, ev, data)
{
  ...
  /* Start TSCH: coordinator on node 1, otherwise join */
  tsch_set_pan_secured(0);
  tsch_set_coordinator(node_id == 1);
  NETSTACK_MAC.on();
  ...
  else
  {
    ...
    sink_ll.u8[LINKADDR_SIZE - 2] = 0;
    sink_ll.u8[LINKADDR_SIZE - 1] = 1; /* 00:01 */
  ...
  while (1)
  {
    ...
    if (node_id == 1 && etimer_expired(&dl_timer))
    {
      ...
      {
        id_ip_t *entry = id_ip_find_by_last(next_dl);
        if (entry)
        {
          uip_ipaddr_t dst;
          uip_ipaddr_copy(&dst, &entry->ip6);
          uint8_t payload[2] = {(uint8_t)(dl_seq >> 8), (uint8_t)(dl_seq & 0xFF)};
          APP_LOG("APP-DL[SINK]: send SR seq=%u -> %02u:%02u\n", (unsigned)dl_seq, (unsigned)entry->id0, (unsigned)entry->id1);
