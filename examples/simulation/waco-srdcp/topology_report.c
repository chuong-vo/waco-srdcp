...
#ifndef LOG_TOPO
#define LOG_TOPO 0 /* 1: enable log; 0: disable log */
#endif
...
/**
 * @brief Callback for the topology report hold timer.
 * @param ptr A pointer to the `my_collect_conn` structure.
 * @details This function is called when the hold timer expires. If a topology
 *          report is still pending (`treport_hold` is set), it sends a dedicated
 *          report packet because it wasn't piggybacked on another message in time.
 */
...
/**
 * @brief Checks if a node address is already present in a topology report block.
 * @param conn Not used (kept for signature consistency).
 * @param node The node address to check for.
 * @param len  The number of entries currently in the report block.
 ...
 */
/**
 * @brief Sends a topology report to the parent node.
 * @param conn    The collect connection structure.
 * @param forward `1` if forwarding a report received from a child, `0` if
 *                originating a new report.
 * @details When forwarding (`forward == 1`), this function may also piggyback the
 *          current node's own pending report to consolidate transmissions.
 */
 ...
 */
/**
 * @brief Processes a received topology report at the SINK.
 * @param conn The collect connection structure (at the SINK).
 * @details This function parses the block of `tree_connection` entries from the
 *          packet, sanitizes them, and updates the SINK's parent dictionary
 *          (`routing_table`) accordingly.
 */
void deliver_topology_report_to_sink(my_collect_conn *conn)
{
        ...
        for (i = 0; i < len; i++)
        {
                ...
                /* Sanitize high byte and discard garbage entries */
                tc.node.u8[1] = 0x00;
                tc.parent.u8[1] = 0x00;
