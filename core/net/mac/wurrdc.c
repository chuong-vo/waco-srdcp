...
send_one_packet(mac_callback_t sent, void *ptr)
{
  ...
  printf("WU Tx: sending wake-up signal to %02x:%02x\n",
         ((linkaddr_t *)&WUR_TX_BUFFER)->u8[0],
         ((linkaddr_t *)&WUR_TX_BUFFER)->u8[1]);
  wur_set_tx(); // send the Wake-up trigger signal, high GPIO
  clock_delay(100);
  wur_clear_tx(); // clear the signal, low GPIO
  clock_delay(1000);
  printf("Main Radio: ON (preparing to send data)\n"); // delay to synchronize with the receiver
...
/* TODO We may want to acknowledge only authentic frames */
  ...
  printf("Main Radio: OFF (finished sending)\n");
  off();
}
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(wur_process, ev, data)
{
  ...
  while (1)
  {
    ...
    {
      printf("WuR Event: received WuS for %02x:%02x\n",
             ((linkaddr_t *)&WUR_RX_BUFFER)->u8[0],
             ((linkaddr_t *)&WUR_RX_BUFFER)->u8[1]);

      // Turn on main radio to receive data
      printf("Main Radio: ON (receiving data)\n");

      on();                  // turn on the radio on the receiver side
      etimer_set(&timer, 3); // timeout for the reception of data packet
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&timer));
      printf("Main Radio: OFF (done receiving)\n");
